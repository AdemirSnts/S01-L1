using System;
using System.Collections.Generic;

// Classe abstrata
abstract class AbominacaoCosmica
{
    public string Nome { get; set; }

    public AbominacaoCosmica(string nome)
    {
        Nome = nome;
    }

    public abstract void Manifestar();
}

// Classe Shoggoth
class Shoggoth : AbominacaoCosmica
{
    public Shoggoth(string nome) : base(nome) { }

    public override void Manifestar()
    {
        Console.WriteLine(Nome + ": A massa de protoplasma se arrasta borbulhando, moldando olhos e órgãos.");
    }
}

// Classe Caçador Alado
class CacadorAlado : AbominacaoCosmica
{
    public CacadorAlado(string nome) : base(nome) { }

    public override void Manifestar()
    {
        Console.WriteLine(Nome + ": A criatura bate suas asas membranosas, mergulhando do vácuo estelar em um voo silencioso e perturbador.");
    }
}

// Classe principal
class Program
{
    static void Main(string[] args)
    {
        // Lista
        List<AbominacaoCosmica> criaturas = new List<AbominacaoCosmica>();

        // Instancias
        criaturas.Add(new Shoggoth("Shoggoth Primordial"));
        criaturas.Add(new CacadorAlado("Caçador Alado"));

        foreach (var criatura in criaturas)
        {
            criatura.Manifestar();
        }
    }
}